...
const cutExtraSpace =()=>{
  ...
  props.showAlert('No extra spaces!','success')
    ...
    <div className="container">
      ...
      <div className=" mb-1 ">
        <textarea className="form-control" style={{backgroundColor:props.mode==='light' ? 'white':'#212529',
    ...
    <div className="container my-2" style={{color:props.mode==='light' ? 'black':'#fcf4f4c9'}}>
      ...
      <p className="mb-0"><b>{text.split(/\s+/).filter((element)=>{return element.length!==0}).length}</b> word(s) and <b>{(text.length-text.split(" ").length)+1} </b>character(s)</p>
      <p className="mb-2 "><b>{0.008 * (text.split(/\s+/).filter((element)=>{return element.length!==0}).length)}</b> minute(s) required for reading the above text</p>
